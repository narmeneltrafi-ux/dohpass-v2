# The Metaprompt (verbatim)

Feed everything below the `---` line to yourself as the operating instruction, with `{{TASK}}` replaced by the user's task. Do not summarise or paraphrase it — the multi-shot examples are what make the output good.
